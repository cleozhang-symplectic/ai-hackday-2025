...
// Define tool type manually to avoid import issues
interface MCPTool {
  name: string;
  description: string;
  inputSchema: {
...
const TOOLS: MCPTool[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        currency: {
          type: 'string',
          enum: ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'AUD', 'CHF', 'CNY', 'INR', 'SGD', 'HKD', 'NZD'],
    ...
    inputSchema: {
      ...
      properties: {
        ...
        currency: {
          type: 'string',
          enum: ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'AUD', 'CHF', 'CNY', 'INR', 'SGD', 'HKD', 'NZD'],
          description: 'New currency (optional)'
        },
        category: {
          type: 'string',
          enum: ['Food', 'Transportation', 'Entertainment', 'Utilities', 'Healthcare', 'Shopping', 'Other'],
    ...
    inputSchema: {
      ...
      properties: {
        convertToCurrency: {
          type: 'string',
          enum: ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'AUD', 'CHF', 'CNY', 'INR', 'SGD', 'HKD', 'NZD'],
          description: 'Currency to convert all amounts to (optional)',
          default: 'USD'
        }
      }
    }
...
class ExpenseTrackerMCPServer {
  ...
  private async handleGetSpendingSummary(args: any) {
    ...
    // Convert amounts if currency conversion is requested
    ...
    if (convertToCurrency !== 'USD') {
      try {
        ...
        // If conversion fails, note it but continue with original amounts
      }
    }
